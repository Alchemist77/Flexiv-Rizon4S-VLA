...
STEPS_ARG = "--steps"
ALL_INSTRUCTIONS = [
    "pick and place the white box",
    "pick and place the green box",
    "pick and place the blue box",
    "pick and place the yellow box",
...
class RizonFastPickPlaceDemo(RizonTaskSpaceDemo):
    ...
    def _instruction_from_assignments(self) -> str:
        ...
        return f"pick and place the {setup['color']} box"

    def reset_random_task(self):
        sampled_positions = self._sample_box_positions()
        slot_order = self.rng.permutation(len(self.PLACE_SLOTS))
        self.current_place_slots = tuple(self.PLACE_SLOTS[i].copy() for i in slot_order)
        self.current_box_setup = []
        ...
        for idx, body_name in enumerate(self.BOX_BODY_NAMES):
            ...
            self.data.qpos[qidx : qidx + 7] = np.array([pos[0], pos[1], pos[2], 1.0, 0.0, 0.0, 0.0])
            ...
            self.current_box_setup.append(
                {
                    ...
                    "pick_pos": pos.copy(),
                    "place_pos": self.current_place_slots[idx].copy(),
                    "slot_label": self.SLOT_LABELS[slot_order[idx]],
                }
            )
        ...
        for idx, setup in enumerate(self.current_box_setup):
            ...
            print(
                f"  {marker} {setup['color']:<6s} pick={np.round(setup['pick_pos'], 3).tolist()} -> place={np.round(setup['place_pos'], 3).tolist()} ({setup['slot_label']})"
            )
    ...
    def _build_trajectory(self):
        hp = self.home_pos
        hq = self.home_quat
        active_place_slots = getattr(self, "current_place_slots", self.PLACE_SLOTS)
        # wps = [Waypoint(hp, hq, 1, "home_init"), Waypoint(hp, hq, 1, "go_home_surely")]
        wps = []
        idx = int(getattr(self, "active_box_index", 0))
        body_id = self.box_body_ids[idx]
        box_pos = self.data.xpos[body_id].copy()
        place_pos = active_place_slots[min(idx, len(active_place_slots) - 1)].copy()
        pre_approach_box = np.array([box_pos[0], box_pos[1], hp[2] - 0.1])
        above_box = np.array([box_pos[0], box_pos[1], box_pos[2] + self.PICK_APPROACH_Z+0.05])
        above_box_v1 = np.array([box_pos[0], box_pos[1], box_pos[2] + self.PICK_APPROACH_Z-0.01])
        grasp_box = np.array([box_pos[0], box_pos[1], box_pos[2] + self.PICK_GRASP_Z])
        lift_box = np.array([box_pos[0], box_pos[1], box_pos[2] + self.PICK_LIFT_Z])
        place_above = np.array([place_pos[0], place_pos[1], place_pos[2] + self.PLACE_APPROACH_Z])
        place_drop = np.array([place_pos[0], place_pos[1], place_pos[2] + self.PLACE_DROP_Z])
        ...
        wps.extend(
            [
                ...
                Waypoint(place_drop, hq, self.PLACE_HOLD_STEPS, f"gripper_open_place{tag}"),
                Waypoint(place_retreat, hq, self.PICK_HOLD_STEPS, f"retreat_after_place{tag}"),
                # Waypoint(hp, hq, 40, "return_home_final"),
            ]
        )
        return TrajectoryPlanner(wps, steps_per_segment=5)
...
class MujocoRizonWrapper:
    ...
    def _get_target_place_pos(self) -> np.ndarray:
        if hasattr(self.demo, "current_box_setup") and self.demo.current_box_setup:
            ...
            return np.asarray(self.demo.current_box_setup[idx]["place_pos"], dtype=np.float32).copy()
        slot = self.demo.PLACE_SLOTS[min(self.place_slot_index, len(self.demo.PLACE_SLOTS) - 1)]
    ...
    def _get_state(self) -> np.ndarray:
        ...
        total = 23
        """
        arm_q = self.data.qpos[self.demo.arm_qidx].copy().astype(np.float32)
        ee_pos = self.data.site_xpos[self.demo.ee_site_id].copy().astype(np.float32)

        box_pos = self._get_box_pos(self.task_box_index).astype(np.float32)
        place_pos = self._get_target_place_pos().astype(np.float32)

        rel_box = (box_pos - ee_pos).astype(np.float32)
        rel_place = (place_pos - ee_pos).astype(np.float32)

        grip = np.array([self.demo._current_gripper_width()], dtype=np.float32)

        state = np.concatenate(
            [arm_q, ee_pos, box_pos, place_pos, rel_box, rel_place, grip],
            axis=0
        )
    ...
    def _set_task_from_instruction(self, instruction: str) -> bool:
        ...
        self.place_slot_index = int(found_idx)
        if hasattr(self.demo, "active_box_index"):
            self.demo.active_box_index = int(found_idx)
        if hasattr(self.demo, "current_instruction"):
            self.demo.current_instruction = f"pick and place the {color_names[found_idx]} box"
        if hasattr(self.demo, "current_box_setup") and self.demo.current_box_setup:
            self.demo.planner = self.demo._build_trajectory()
        return True
    ...
    def reset(self, seed=None, instruction=None):
        ...
        if not used_instruction_task:
            if hasattr(self.demo, "active_box_index"):
                ...
                self.place_slot_index = int(self.demo.active_box_index)
